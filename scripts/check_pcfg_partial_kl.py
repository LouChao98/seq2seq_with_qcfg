...
def kl_partial(params, params2, x, n, nt, pt):
    ll = []
    ll2 = []
    for tree in enumerate_tree(0, n, nt, pt):
        tree = [(l, r - 1, t) for l, r, t in tree]
...
params2 = {
    "term": torch.randn(B, TGT_PT, VOCAB).log_softmax(-1),
    "root": torch.randn(B, TGT_NT).log_softmax(-1),
    "rule": torch.randn(B, TGT_NT, (TGT_NT + TGT_PT) ** 2)
    .log_softmax(-1)
    .view(B, TGT_NT, TGT_NT + TGT_PT, TGT_NT + TGT_PT),
}
terms = params2["term"].unsqueeze(1).expand(B, n, TGT_PT, params2["term"].size(2))
terms = torch.gather(terms, 3, x_expand).squeeze(3)
params2["term"] = terms

params_reduced = {}
params_reduced["term"] = params["term"].view(B, TGT_)
