...
class ScanModule(pl.LightningModule):
    ...
    def forward_visualize(self, batch, sample=False):
        ...
        tgt_spans, aligned_spans = self.decoder(
...
class NeuralQCFGTgtParser(nn.Module):
    ...
    def get_params(self, node_features, spans, x=None, x_str=None):
        ...
        # TODO: copy mechanism
        # NOTE: make sure the params have shape (nt/pt_states, nt/pt_spans)
        if self.use_cpd:
            return self.get_params_cpd(node_features, spans, x, x_str)
        else:
            return self.get_params_basic(
                node_features, spans, x, x_str, ignore_src=False
            )

    def get_params_basic(
        self, node_features, spans, x=None, x_str=None, ignore_src=False
    ):
        ...
        for spans_inst, node_features_inst in zip(spans, node_features):
            ...
            if len(nt_node_feature) == 0:
                assert False  # TODO check when will this be triggered
