...
class GeneralSeq2SeqCRFModule(ModelBase):
    ...
    * custom test metric
    ...
    def setup(self, stage: Optional[str] = None, datamodule=None) -> None:
        ...
        # TODO this is a workaround. Add <bos> <eos> before embedding should be better
        # pretrained model also need modification because <bos> <eos> (and '_') are accumulated to position 0
        self.bos_embedding = nn.Parameter(torch.randn(input_dim))
        self.eos_embedding = nn.Parameter(torch.randn(input_dim))
        self.encoder = instantiate(self.hparams.encoder, input_dim=input_dim)

        self.parser: SrcParserBase = instantiate(self.hparams.parser, n_encoder_hidden=self.encoder.get_output_dim())
    ...
    def encode(self, batch):
        ...
        x = torch.cat([x.new_zeros(x.shape[0], 1, x.shape[2]), x, x.new_zeros(x.shape[0], 1, x.shape[2])], dim=1)
        x[:, 0] = self.bos_embedding.unsqueeze(0)
        x[torch.arange(len(src_lens)), [item + 1 for item in src_lens]] = self.eos_embedding.unsqueeze(0)
    ...
    def forward(self, batch):
        ...
        with self.profiler.profile("src_seq_encoding"):
            x = self.encode(batch)

        with self.profiler.profile("compute_src_sampling"):
            dist = self.parser(x, src_lens)
            src_event, src_logprob = self.parser.sample(x, src_lens, dist=dist)
            src_spans = src_event["span"]

        with self.profiler.profile("src_tree_encoding"):
        ...
        if self.current_epoch < self.hparams.warmup_pcfg:
            ...
            with self.profiler.profile("reward"), torch.no_grad():
                src_spans_argmax = self.parser.argmax(x, src_lens, dist=dist)
        ...
        if self.training and self.current_epoch >= self.hparams.warmup_qcfg:
            ...
            if (e := self.hparams.parser_entropy_reg) > 0:
                entropy = self.parser.entropy(x, src_lens, dist)
        ...
        return {
            ...
            + pr_neq_nt_reg
            + pt_prior_loss,
            "encoder": objective + src_entropy_reg,
            "tgt_nll": tgt_nll,
    ...
    def forward_visualize(self, batch):
        ...
        x = self.encode(batch)
        src_spans = self.parser.argmax(x, src_lens)
        src_annotated = []
    ...
    def forward_generate(self, batch, get_baseline=False):
        ...
        x = self.encode(batch)
        dist = self.parser(x, src_lens)
        src_spans = self.parser.argmax(x, src_lens, dist=dist)
