"""https://github.com/FranxYao/RDP/blob/dbf313ebde4a34ec0e538b58e6bdb0e14b1a8335/src/frtorch/structure/linear_chain_crf.py
===============
...
class LinearChainCRF(nn.Module):
  ...
  forward backward, evaluation, and viterbi"""
    ...
    def normalize(self, transition, emission):
        ...
        elif self.potential_normalization == "none":
            pass
        else:
            raise ValueError(
                "Invalid potential normalization method %s"
    ...
    """
        ...
        if len(transition.size()) == 2:
            log_potentials = transition.view(1, 1, num_state, num_state).expand(
                batch_size, seq_len, num_state, num_state
            ) + emission.view(batch_size, seq_len, 1, num_state).expand(
                batch_size, seq_len, num_state, num_state
            )
        else:
            log_potentials = transition + emission.view(
                batch_size, seq_len, 1, num_state
            ).expand(batch_size, seq_len, num_state, num_state)
        return log_potentials

    def forward_topk(self, state_matrix, emission_potentials, seq_lens, sum_size):
        ...
        #   + transition_bias
        sampled_transition = torch.matmul(
            sampled_transition[:, :-1], sampled_transition[:, 1:].transpose(2, 3)
        )
        ...
        log_potentials = torch.zeros(
            batch_size, max_len, num_state_sampled, num_state_sampled
        ).to(device)
        log_potentials[:, 1:] = sampled_transition + combined_emission[:, 1:].unsqueeze(
            2
        )
    ...
    def sample_states(
        ...
        * Randomized FFBS
        ...
        if proposal == "softmax":
            ...
            proposal_p = 1.0 + torch.zeros_like(emission_potentials)
        else:
    ...
    """
        ...
        if not return_sampled_idx:
            return log_Z_est
        else:
            return log_Z_est, sampled_index
    ...
    def seq_log_potential(
        self, seq, transition_potentials, emission_potentials, seq_lens
    ):
        ...
        for i in range(max_len):
            if i == 0:
                score[:, i] += tmu.batch_index_select(
                    emission_potentials[:, 0], seq[:, 0]  # [batch, num_state]
                )  # [batch]
            else:
                ...
                score[:, i] += transition_
                score[:, i] += tmu.batch_index_select(
                    emission_potentials[:, i], current_ind
                )
    ...
    def rsample(
        ...
        sample_prob = sample_prob.masked_fill(mask == 0, 1.0)
