...
class Decomp1Left(DecompBase):
    ...
    def inside(self, params, semiring, trace=False):
        ...
        TLNT = L[..., : self.nt_states]
        TLPT = L[..., self.nt_states :]
        TRNT = R[..., : self.nt_states]
        TRPT = R[..., self.nt_states :]
        ...
        term = term.view(list(term.shape[:4]) + [1, self.pt_states, -1])
        d = np.log(term.shape[-1])
        # cxmn1 tp sp,cx11r tp 1 -> cxmnr
        left_term = semiring.sum(semiring.mul(term, TLPT[:, :, None, None, :, :, None] - d).flatten(5), dim=5)
        right_term = semiring.sum(semiring.mul(term, TRPT[:, :, None, None, :, :, None] - d).flatten(5), dim=5)

        # c b nt r  * c b nt
        root = semiring.sum(semiring.mul(head, root.unsqueeze(3)), dim=2)
        # c b tgtnt srcnt r -> c b 1 r tgtnt srcnt
        head = head.view(semiring.size, bsz, self.nt_states, -1, rank).movedim(4, 2).unsqueeze(2)
        d = np.log(head.shape[5])
        # c b r 1 tgt_nt 1 * c b 1 r tgtnt srcnt = batch, parentr, childr
        HL = semiring.sum(semiring.mul(TLNT[:, :, :, None, :, None] - d, head).flatten(4), dim=4)
        HR = semiring.sum(semiring.mul(TRNT[:, :, :, None, :, None] - d, head).flatten(4), dim=4)
...
def convert_decomp1copypt_to_pcfg(p: Dict[str, torch.Tensor], tgt_nt):
    print("Conversion only support 1-gram PT.")
    tgt_pt = p["left"].shape[2] - tgt_nt
    src_nt = p["head"].shape[1] // tgt_nt
    src_pt = p["term"].shape[3] // tgt_pt
    size = torch.tensor([src_nt] * tgt_nt + [src_pt] * tgt_pt)
    left = p["left"].repeat_interleave(size, dim=2)
    right = p["right"].repeat_interleave(size, dim=2)
    size = size.repeat_interleave(size)[None, None]
    rule = torch.einsum("xar,xrb,xrc->xabc", p["head"].exp(), left.exp() / size, right.exp() / size)
    rule = (rule + 1e-9).log()
...
if __name__ == "__main__":
    ...
    params = Decomp1Left.random(B, N, TGT_PT, SRC_PT, TGT_NT, SRC_NT, r, ngram=ngram)
    ...
    pcfg = Decomp1Left(params, lens, **meta)
    pcfg_ref = NoDecomp(params_ref, lens, **meta)
