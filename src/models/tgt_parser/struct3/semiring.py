...
class UnnormalizedCrossEntropySemiring(SemiringBase):
    zero = torch.tensor([-1e9, -1e9, 0.0])
    one = torch.tensor([0.0, 0.0, 0.0])
    size = 3
    ...
    def sum(a, dim):
        ...
        log_sm_q = a[1]
        sm_p = log_sm_p.exp()
        return torch.stack((part_p, a[1], torch.sum(a[2].mul(sm_p) - log_sm_q.mul(sm_p), dim=dim)))
    ...
    def normal_space_sum(a, dim):
        ...
        sm_p = a[0] / (part_p.unsqueeze(dim) + 1e-9)
        sm_q = a[1]
        return torch.stack((part_p, a[1], torch.sum(a[2].mul(sm_p) - sm_q.log().mul(sm_p), dim=dim)))
