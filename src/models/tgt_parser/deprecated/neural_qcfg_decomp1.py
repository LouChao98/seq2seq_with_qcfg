...
class NeuralQCFGDecomp1TgtParser(TgtParserBase):
    def __init__(
        ...
        self.pcfg = Decomp1(self.nt_states, self.pt_states)
        self.vocab = vocab
        self.dim = dim
        ...
        self.rank_proj_head = nn.Sequential(nn.LeakyReLU(), nn.Linear(dim, cpd_rank))  # , nn.LayerNorm(cpd_rank)
        self.rank_proj_left = nn.Sequential(nn.LeakyReLU(), nn.Linear(dim, cpd_rank))  # , nn.LayerNorm(cpd_rank)
        self.rank_proj_right = nn.Sequential(nn.LeakyReLU(), nn.Linear(dim, cpd_rank))  # , nn.LayerNorm(cpd_rank)
    ...
    def get_params(
        ...
        rule_emb_left = rule_emb_left.transpose(1, 2).softmax(-1)
        rule_emb_right = rule_emb_right.transpose(1, 2).softmax(-1)
