...
from supar.modules.affine import Triaffine

from ..components.common import MultiResidualLayer
from .base import DirTgtParserPrediction, TgtParserBase
from .struct.decomp5 import Decomp5, Decomp5Sampler
...
class NeuralDecomp5TgtParser(TgtParserBase):
    def __init__(self, cpd_rank=32, direction=0, vocab=100, dim=256, num_layers=3, src_dim=256, **kwargs):
        ...
        self.rijk_weight = nn.Parameter(torch.empty(nt_states, dim, dim))
    ...
    def forward(self, node_features, spans, **kwargs):
        ...
        valid_mask = valid_mask.unsqueeze(1).expand(-1, self.nt_states, -1, -1, -1)
        ...
        if self.rule_hard_constraint is not None:
            ...
            mask = mask.unsqueeze(1).expand(-1, self.nt_states, -1, -1, -1)
    ...
    def observe_x(self, pred: DirTgtParserPrediction, x, lengths, inplace=True, **kwargs) -> DirTgtParserPrediction:
        ...
        pred.dist = Decomp5(pred.posterior_params, pred.lengths, **pred.common())
        return pred
    ...
    def prepare_sampler(self, pred: DirTgtParserPrediction, src, src_ids, inplace=True) -> DirTgtParserPrediction:
        ...
        pred.sampler = Decomp5Sampler(pred.params, **pred.common(), **self.sampler_common())
        return pred
