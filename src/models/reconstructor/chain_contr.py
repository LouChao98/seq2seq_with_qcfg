...
class ChainContrastiveReconstructor(ReconstructorBase):
    # sample
    def __init__(
        self,
        lm,
    ...
    def forward(self, batch, tgt_pred):
        ...
        tgt_words = self.tgt_mlp(h)
        src_words = self.src_mlp(tgt_pred.pt_features)
        score = self.scorer(tgt_words, src_words)  # b x from x to
        ...
        loss_recon = (score.max(2).values - score.diagonal(0, dim1=1, dim2=2) + 1).clamp(0) * mask
        loss_recon = loss_recon.sum(1)
