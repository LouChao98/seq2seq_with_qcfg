# based on https://github.com/berlino/btg-seq2seq/blob/master/neural_btg/models/btg_seg2seg.py
...
from neural_btg.structs.segre import BTGRule, Seg, Segre, _check_max_segment_len
...
class BTGSegWithSrcTgt:
    ...
    def build_src_seg_id_and_mask(ins_src_ids, seg_sos_id, seg_eos_id, src_seg_s, src_seg_e, src_len):
        ...
        ins_src_seg_mask = [1] + [0] * src_seg_s + (src_seg_e - src_seg_s) * [1] + [0] * (src_len - src_seg_e) + [1]
    ...
    def _obtain_tgt_neg_logprob(self, ins_idx, segre_offset, seg_loss_v, pair_num_cs):
        ...
        src_segre_sample, tgt_segre_sample = (
            self.id2src_segre[ins_idx][segre_offset],
            self.id2tgt_segre[ins_idx][segre_offset],
        )
        single_pair_loss = []
        for seg_idx, (src_segment, tgt_segment) in enumerate(
            zip(
    ...
    def aggregate_loss_elbo(self, seg_loss_v):
        ...
        for ins_idx in range(self.bs):
            ...
            with torch.no_grad():
                ...
                baseline_logprob = self._obtain_tgt_neg_logprob(
                    ins_idx, baseline_segre_offset, seg_loss_v, pair_num_cs
                )
            ...
            for segre_offset in range(len(self.id2src_segre[ins_idx]) - 1):
                ...
                entropy_reward = (
                    (tgt_segre_sample.cond_entropy - baseline_tgt_segre_sample.cond_entropy).detach().item()
                )
                ...
                logger.debug(
                    f"instance {ins_idx} got reward {btg_reward} from {segre_offset}th segre with tgt logprob {tgt_logprob.item()}, btg logprob {btg_log_prob.item()}"
                )
                logger.debug(f"instance {ins_idx} got entropy reward: {entropy_reward}")

                elbo_obj = (
                    btg_reward * btg_log_prob
                    + tgt_logprob
                    + entropy_reward * src_seg_reorder_logprob
                    + tgt_segre_sample.cond_entropy
                )
...
class BTGSeg2Seg(torch.nn.Module):
    ...
    def __init__(
        self, seq2seq, config, sos_id, eos_id, seg_sos_id, seg_eos_id, pad_id, beam_size, use_posterior=False
    ):
        """
        Args:
        ...
        """
        super().__init__()
        self.seq2seq: Union[BartForConditionalGeneration, T5ForConditionalGeneration] = seq2seq
        self.device = self.seq2seq.device
            ...
            for k in range(num_batch):
                ...
                if normalize_score:
                    ...
                    trs_bs = self.seq2seq.compute_transition_beam_scores(
                        sequences=bart_ret.sequences, scores=bart_ret.scores, beam_indices=bart_ret.beam_indices
                    )
    ...
    def btg_train(
        ...
        for ins_idx in range(bs):
            ...
            for segre_sample in segre_samples:
                ...
                logger.debug(
                    f"instance {ins_idx}: sampled reordered src segments {segre_sample.obtain_reordered_segments()}, prob {segre_sample.tree_log_prob().exp().item()}"
                )

                tgt_segre_sample = tgt_seg.cond_sample(
                    ins_idx,
                    num_segments,
                    1,
                    segre_sample,
                ...
                logger.debug(
                    f"instance {ins_idx}: sampled tgt segments {tgt_segre_sample.obtain_orig_segments()}, prob {tgt_segre_sample.tree_log_prob().exp().item()}"
                )
            ...
            logger.debug(
                f"instance {ins_idx}: argmax src segments {argmax_src_segre_sample.obtain_reordered_segments()}, prob {argmax_src_segre_sample.tree_log_prob().exp().item()}"
            )
            ...
            logger.debug(
                f"instance {ins_idx}: argmax tgt segments {argmax_tgt_segre_sample.obtain_orig_segments()}, prob {argmax_tgt_segre_sample.tree_log_prob().exp().item()}"
            )
    ...
    def btg_marginalize_decode(
            ...
            for idx, (cand, cur_score) in enumerate(cur_l):
                if cand == candidate_seq:
                    try:
                        cur_l[idx] = (
                            candidate_seq,
                            math.log(math.exp(cur_score) + math.exp(candidate_score) + EPSILON),
                        )
        ...
        src_seg_masks_l = []
        src_seg2idx = {}
        for seg_idx, (src_seg_s, src_seg_e) in enumerate(
            segre.obtain_effective_segments(num_segments, argmax=use_argmax)
        ):
            if src_seg_s == 0 and src_seg_e == src_len:
                # seq2seq decoding will be done later
                continue
            else:
                seg_src_ids, seg_src_mask = self.BTG_class.build_src_seg_id_and_mask(
                    src_ids[bs_idx], self.seg_sos_id, self.seg_eos_id, src_seg_s, src_seg_e, src_len
                )
        ...
        seg_beams, seg_beams_scores = self._seq2seq_inference(
            src_seg_ids, src_seg_masks, normalize_score=True, seg_gen=True
        )
                                ...
                                for left_candidate, left_candidate_score in left_candidates:
                                    for right_candidate, right_candidate_score in right_candidates:
                                        if parent_nt == A:
                                            ...
                                            straight_candidate_score = (
                                                left_candidate_score + right_candidate_score + rule_score
                                            )
                                            merge_score(
                                                chart[n, s, e, A], straight_candidate, straight_candidate_score
                                            )
                                        else:
                                            assert parent_nt == B
                                            invert_candidate = merge_candidate(right_candidate, left_candidate)
                                            invert_candidate_score = (
                                                left_candidate_score + right_candidate_score + rule_score
                                            )
        ...
        for n in range(1, num_segments + 1):
            ...
            if n == 1:
                ...
                A_candidates = [
                    (cand, cand_score + segre.pcfg_cache["root", n][bs_idx, A])
                    for cand, cand_score in chart[n, 0, src_len, A]
                ]
                B_candidates = [
                    (cand, cand_score + segre.pcfg_cache["root", n][bs_idx, B])
                    for cand, cand_score in chart[n, 0, src_len, B]
                ]
