    ...
    def build_tree_str(self, actions, tokens, tags, subword_end_mask=None):
        ...
        for a in actions:
            if self.is_nt(a):
                ...
                if (subword_end_mask is None) or (subword_end_mask is not None and subword_end_mask[subword_i]):
...
class InOrderActionDict(TopDownActionDict):
    ...
    def __init__(self, nonterminals):
        ...
        self.i2a = ["<pad>", "SHIFT", "REDUCE", "FINISH"] + ["NT({})".format(nt) for nt in nonterminals]
        self.a2i = dict([(a, i) for i, a in enumerate(self.i2a)])
    ...
    def build_tree_str(self, actions, tokens, tags, subword_end_mask=None):
        ...
        def end_of_subword():
            return (subword_end_mask is None) or (subword_end_mask is not None and subword_end_mask[subword_i])
