...
class InOrderRNNG(TopDownRNNG):
    def __init__(
        ...
        super(InOrderRNNG, self).__init__(
            action_dict,
            vocab,
            padding_idx,
            w_dim,
    ...
    def update_stack_rnn(self, states, actions, shift_idx, shifted_embs):
        ...
        if nts.size(0) > 0:
            ...
            for i, b in enumerate(nt_idx):
                new_stack_tops_b = [
                    [[layer[0][i], layer[1][i]] for layer in nt_stack_top_context],
...
class InOrderState(TopDownState):
    def __init__(
        ...
        super(InOrderState, self).__init__(pointer, stack, stack_trees, nopen_parens, ncons_nts, nt_index, nt_ids)
