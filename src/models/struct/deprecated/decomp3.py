...
class Decomp3(DecompBase):
    ...
    def inside_with_fused_factor(self):
        params = self.params
        term = params["term"]  # (batch, seq_len, PT)
        root = params["root"]  # (batch, NT)
        head = params["head"]  # (batch, NT, r), A[i] -> R
    ...
    def random_dir0(bsz, max_len, tgt_pt, src_pt, tgt_nt, src_nt, r):
        nt = tgt_nt * src_nt
        pt = tgt_pt * src_pt
        slr = torch.rand(bsz, r, src_nt, src_nt + src_pt, src_nt + src_pt)
...
class Decomp3Sampler:
    def __new__(cls, *args, direction, **kwargs):
        if direction == 0:
            return Decomp3Dir0Sampler(*args, **kwargs)
        elif direction == 1:
            return Decomp3Dir1Sampler(*args, **kwargs)
        raise ValueError


class Decomp3Dir0Sampler(DecompSamplerBase):
    @torch.no_grad()
...
class Decomp3Dir1Sampler(DecompSamplerBase):
    @torch.no_grad()
    def process_params(self, params: Dict[str, Tensor]):
        terms = params["term"].exp().cumsum(2).cpu().numpy()
        roots = params["root"].exp().cumsum(1).cpu().numpy()
...
def convert_decomp3_to_pcfg(p, nt_states):
    B, _, r = p["head"].shape
    TGT_NT = nt_states
    TGT_PT = p["left"].shape[-1] - TGT_NT
    head = p["head"].view(B, nt_states, -1, r)
    SRC_NT = head.shape[2]
    SRC_PT = p["slr"].shape[-1] - SRC_NT
    rule11 = torch.einsum(
