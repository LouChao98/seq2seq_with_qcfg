...
class Decomp7Impl3(DecompBase):
    ...
    def inside(self, trace) -> Tuple[Tensor, Tensor]:
        ...
        if trace:
            span_indicator = term.new_zeros(batch, N, N, self.nt_states, nt_spans, requires_grad=True)
            span_indicator_running = span_indicator
        else:
            span_indicator = None
            span_indicator_running = None

        left_s = term.new_full((batch, N, N, nt_spans, R), -1e9)
        right_s = term.new_full((batch, N, N, nt_spans, R), -1e9)
        ...
        for step, w in enumerate(range(2, N)):
            ...
            if w == 2:
                ...
                y = stripe(left_s, n, w - 2, (0, 2)).clone()
                z = stripe(right_s, n, w - 2, (1, w), 0).clone()
                if w == 3:
...
def convert_decomp7impl3_to_pcfg(params, nt_states):
    params = {**params}
...
@checkpoint
@torch.jit.script
def merge_h2(y, z, y_term, z_term, sl1r, slr1, h):
    qnkrj1 = eq_qnkrj(y_term, z)
    qnkrj3 = eq_qnkrj(y, z_term)
    # normalizer = torch.max(qnkrj1.amax((2, 3, 4)), qnkrj3.amax((2, 3, 4)))
...
@checkpoint
@torch.jit.script
def merge_h3(y, z, y_term, z_term, sl1r, slr1, slr, h):
    qnkrj1 = eq_qnkrj(y_term, z[:, :, :1])
    qnkrj2 = eq_qnkrj(y[:, :, :-1], z[:, :, 1:])
    qnkrj3 = eq_qnkrj(y[:, :, -1:], z_term)
    maxes = [
        qnkrj1.amax((2, 3, 4)),
        qnkrj2.amax((2, 3, 4)),
        qnkrj3.amax((2, 3, 4)),
