...
class NeuralTNPCFGSrcParser(SrcParserBase):
    def __init__(
        self,
        vocab=100,
        dim=256,
        ...
        super(NeuralTNPCFGSrcParser, self).__init__()
        self.neg_huge = -1e5
        self.vocab = vocab
    ...
    def get_rules(self, x: torch.Tensor):
        ...
        left = left.T.log_softmax(-1)
        right = right.T.log_softmax(-1)
        ...
        # breakpoint()
        return {"term": term.view(b, n, 1, -1), "root": roots, "left": left, "right": right, "head": head}
    ...
    def marginals(self, x, lengths, dist: Optional[Decomp1] = None, extra_scores=None):
        raise NotImplementedError

    @torch.enable_grad()
    def sample(self, x, lengths, dist: Optional[Decomp1] = None, extra_scores=None):
        if dist is None:
    ...
    @torch.enable_grad()
    def gumbel_sample(self, x, lengths, temperature, dist: Optional[Decomp1] = None, extra_scores=None):
        if dist is None:
            dist = self(x, lengths, extra_scores)
    ...
    @torch.enable_grad()
    def argmax(self, x, lengths, dist: Optional[Decomp1] = None, extra_scores=None):
        if dist is None:
            dist = self(x, lengths, extra_scores)
    ...
    @torch.enable_grad()
    def entropy(self, x, lengths, dist: Optional[Decomp1] = None, extra_scores=None):
        if dist is None:
            dist = self(x, lengths, extra_scores)
